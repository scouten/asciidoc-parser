...
The specification is not guaranteed to result in a single, unified grammar (as it may not even be possible to do so).
...
Specifically, AsciiDoc is a lightweight markup language that is predominantly line-oriented and left-aligned.
...
* Two consecutive line endings creates an empty line.
 ...
 ** A converter may choose to replace these line endings with the space character; however, they must be maintained in the parse tree.
* Block boundaries and block metadata are always defined on their own line.
...
* AsciiDoc is a left-aligned language.
* Being left-aligned makes moving content around easier, avoids the problem of excessive/compound indentation, and encourages the author to be more explicit about nesting.
...
** In the named macro form, the macro name is followed by a macro name delimiter followed by a target followed by a boxed attrlist.
...
* A few macros have a shorthand form in addition to a named form (e.g., `+<<target>>+` and `+xref:target[attrlist]+` ).
...
** List markers may be indented, however, the wrapped lines may not be indented because AsciiDoc is left-aligned.
 ...
 ** Refer to macros expressed using non-named syntax as a shorthand macro; still a macro, just not expressed that way
...
* In the past, this has been refered to as "text formatting" or "formatted text" (though formatted text are not limited to this group of syntax).
...
* cross references (xref macro, xref shorthand)
* footnote
* index terms (indexterm, indexterm2, index term shorthands)
...
* The referencing system consists of referencable locations (references identified by ID), a catalog of references, internal references to another location in the same document (xrefs), and external references to other resources.
* References are referencable locations within a document.
 ...
 ** or should it be an object that contains a) the referencable location, b) the concrete location, c) a resource reader
