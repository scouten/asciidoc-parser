...
. It acts as catalog of questions, clarifications, and issues that need to be discussed and resolved during the development of the specification.
...
* *FI* Will any, all, or select Unicode escape sequences be recognized?
...
* *FI* How are space characters in the source going to be handled? What space characters are permitted? How are they going to be preserved?
...
** *FI* Should the underscore no longer be treated as a word character and be treated as a syntax token only?
 ...
 ** Preserving line endings in the parse tree makes it easier to express scenarios in the TCK; *FI* this requirement is still being debated.
...
** Some scenarios may not be possible using the author line; in such cases the attributes will need to be explicitly set
* Revision line
...
*** *FI* Should we standardized the boolean document attribute typed value `true`?
...
* *FI* Should we disallow spaces in attribute names in an attribute entry?
* *FI* Should we allow document attributes to be set inside a delimited block?
...
** *SDR* Confirm the terms and their definitions for what we should use to refer to the list of attributes in raw versus parsed form.
...
** *FI* Should we standardize on the `%notitle` option to hide a section title? (currently Asciidoctor only supports `%untitled`)
...
** *FI* Propose the nestable open block using the standard block delimiter length  (`+~~~~+`).
* no style
...
Should it be an allowed style on any other blocks?
* content model: compound
...
* *FI* Make target forbidden to align menu with other UI macros.
...
* line
 ** *FI* Propose that lines that do not have a reserved designation (a delimiter line or a block attribute line) may have a trailing line comment; we are considering whether this can be loosened to all non-verbatim lines.
* block
