...
// or should we use the term "Space characters"?
=== Spacing characters
...
//* Elements are distinguished by name, where a name represents a major set of traits (e.g., an image), which may be specialized by a variant that shares many traits of the element name.
...
* Macros are a special form of a block or inline element.
...
* Must proceed the implicit metadata lines.
...
* When set in the document header, the attribute is is referred to as document header attribute.
// Q: should we remove built-in and user-defined to own section(s)?
...
* Some document attributes are booleans.
...
These attributes allow the user to define named, reusable content or to act as switched for preprocessor conditionals or extensions.
...
* Control or configure common features in a document (e.g., `toc`).
* Provide a means to populating document metadata (e.g., `author`).

==== Global block attributes and options

=== Attribute naming conventions
// define naming conventions for built-in attributes; identify reserved and recommended prefixes

* Attribute names must be at least one character, must start with a letter, number, or underscore, and may contain any amount of letters, numbers, underscores, or hyphens beyond the first character.
* Attribute names should only contain lowercase letters, though uppercase characters are permitted.
* The casing of attribute names is not modified.
...
* A name identifies the primary function of the element.
* The name often implies the content model.
* Each block name provides semantics not covered by any other block name.
* The block name may be implicit or explicit.
* If a block name is not specified, it is inferred by its source form (e.g., block delimiter, marker, implicit boundaries).
...
NOTE: The TOC must be represented in the ASG so that we can validate the the processor correctly identified entries and their titles and filled it in to the specified number of levels.
...
* A literal paragraph can be interpreted just like a paragraph.
...
* A specialization of an unordered list
* Implicit created if at least one item in the unordered list is a checklist item
...
* The primary text of a dlist item can start on the next line, unlike for regular lists.
...
* *FI* Does an AsciiDoc table cell need to be marked on the cell itself in order to full honor the semantics of the preprocessor; in other words, when marked on the colspec, it will be parsed as a normal cell, then promoted to an AsciiDoc table cell; thus, the behavior could be different
...
* Passthroughs should be use sparingly as they can compromise the portability of an AsciiDoc document.
...
* All formatted text is a span; but not all spans are formatted text <= is this still true?
...
=== Open (Highlighted, formatted with custom role, or unformatted)
...
// NOTE this applies to block-level elements too, so perhaps it needs to be repeated in Blocks or put outside of Blocks and Inlines
