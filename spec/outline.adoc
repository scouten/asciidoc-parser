...
* They are set and assigned a value in the document header or body using an attribute entry.
* If an attribute is set, it has a non-null (non-undefined) value; if an attribute is unset, it is absent; an attribute cannot be set and have a null/undefined value
* When set in the document header, they are referred to as document header attributes.
...
* Attribute names must be at least one character, must start with a letter, number, or underscore, and may contain any amount of letters, numbers, undercores, or hyphens beyond the first character.
