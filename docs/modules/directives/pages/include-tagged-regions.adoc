...
IMPORTANT: The `tag::tagName[]` and `end::tagName[]` directives have to follow a word-boundary and precede a space character or the end of line. The tag name has to consist exclusively of non-whitespace characters and cannot be empty.
Typically, the directives will be placed after a line comment as defined by the language of the source file.
