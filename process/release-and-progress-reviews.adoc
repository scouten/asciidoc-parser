...
It does need to meet all of the following requirements to be considered for release as a patch version of the specification:
...
Prior to creating feature complete milestone builds in preparation for the final specification release, the project will release milestone builds labeled as _0.x.y_ (e.g., 0.1.5, 0.2.0).
...
These milestone builds will use a zero in the first position (i.e., 0.x.y).
The x will increment when a significant feature is added to the specification or its TCK (e.g., 0.3.0. 0.4.0).
The y will increment when the build only contains improvements, changes, or fixes to existing features (e.g., 0.3.2. 0.4.1).
...
Once the project team determines that the specification is approaching feature completeness, the _0.x.y_ pattern will cease being used. At that time, all future milestone builds will use semantic versioning as the project team prepares for releasing version 1.0.0 of the specification (e.g., 1.0.0-alpha.1).
...
.. Shows evidence that one or more compatible implementations meet all of the TCK's requirements and optional aspects.
